...
function Model_V3(
    ...
    z2_on_init::Int = 0,
    z3_on_init::Int = 0,
    setpoint_s1_init::Float64 = 0.0,
    setpoint_s2_init::Float64 = 0.0,
    setpoint_s3_init::Float64 = 0.0,
...
NT = length(Power)
history = 1   # ALWAYS 1 IN THIS MPC FORMULATION
...
@constraint(model, z1_on[1] == z1_on_init)
@constraint(model, z2_on[1] == z2_on_init)
@constraint(model, z3_on[1] == z3_on_init)

@expression(model, z1_OFF[t=1:NT], 1 - z1_on[t])
@expression(model, z2_OFF[t=1:NT], 1 - z2_on[t])
@expression(model, z3_OFF[t=1:NT], 1 - z3_on[t])

# If error flag = true, stack must be OFF for all t
@constraint(model, [t=1:NT], z1_on[t] <= 1 - Int(err1))
@constraint(model, [t=1:NT], z2_on[t] <= 1 - Int(err2))
@constraint(model, [t=1:NT], z3_on[t] <= 1 - Int(err3))
...
@constraint(model, setpoint_s1[1] == setpoint_s1_init)
@constraint(model, setpoint_s2[1] == setpoint_s2_init)
@constraint(model, setpoint_s3[1] == setpoint_s3_init)
...
@objective(model, Max,
    weights.w1 * sum((1-err1)*H2_s1[t] + (1-err2)*H2_s2[t] + (1-err3)*H2_s3[t] for t in history+1:NT) +
